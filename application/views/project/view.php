...
<?php switch ($mode):
  ...
  <?php header("location:" . APP_URL . "/project/search");
    exit; // Move this inside the switch case block
  default: ?>
    <div class="container">
      <div class="row">
              ...
              <table>
                <tr>
                  <?php foreach (explode(",", $row['tag']) as $tag) : ?>
                    <td> <!-- Added custom class for margin -->
    ...
    <div class="container">
      <div class="main-content">
        ...
        <div class="col-md-8" id="projinfo">
          <div class="row">
            <div class="progress">
              <?php
              ...
              ?>
              <div class="progress-bar progress-bar-<?php if (isset($failed)) {
                                                      echo "danger";
                                                    } else {
                                                      echo "success";
                                                    } ?>" role="progressbar" aria-valuenow="<?php echo $progress; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $progress; ?>%">
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <?php if ($row['status'] == 'crowdfunding') : ?>
              ...
              <p class="text-muted">Remaining Amount: $<?php echo $row['minamount'] - $row['curamount']; ?> &nbsp;&nbsp;&nbsp; Total Pledged Amount: $<?php echo $row['minamount']; ?></p>
        ...
        <?php switch ($row['status']):
          case 'crowdfunding': ?>
            <form class="form-inline" action="<?php echo APP_URL ?>/project/pledge" method="post">
              <div style="width: 100%" class="form-group <?php if (isset($pledgeError)) {
                                                            echo 'has-error';
                                                          } ?>">
          ...
          case 'completed': ?>
            ...
            <?php if ((isset($hasPledged) && $hasPledged) && (!isset($hasRated) || !$hasRated)) : ?>
              <form class="form-inline" action="<?php echo APP_URL ?>/project/rate" method="post">
                <div style="width: 100%" class="form-group <?php if (isset($rateError)) {
                                                              echo 'has-error';
                                                            } ?>">
                  <div style="width: 100%" class="input-group">
                    <input type="hidden" name="pid" value="<?php echo $row['pid']; ?>">
      ...
      <!-- <div class="row" id="sample">
        ...
        <?php if (isset($samples)) : ?>
          ...
          <?php endforeach; ?>
        <?php else : ?>
          <p class="text-center">No sample yet</p>
        <?php endif; ?>
      </div> -->

      <div class="row" style="padding-top:20px; margin-top:10px">
        <?php if (isset($comments)) : ?>
          <?php foreach ($comments as $comment) : ?>
            <div class="row comment">
              <h4 class="col-md-2"><strong><a href="<?php echo APP_URL . "/user/view/" . $comment['uname']; ?>"><?php echo $comment['uname']; ?></a></strong>
                <span class="help-block" style="font-size:14px"><?php echo $comment['time']; ?></span>
              </h4>
